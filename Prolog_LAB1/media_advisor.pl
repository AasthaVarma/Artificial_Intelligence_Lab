...
stimulus_situation_visual(diagrams).
...
(lecture_tutorial(X,Y,Z)->writeln('lecture tutorial'));
(videocassette(X,Y,Z)->writeln('videocassette'));
(role_play_exercise(X,Y,Z)->writeln('role play exercise')).

job(building).
job(repairing).
job(troubleshooting).

hands_on(X):- job(X)->writeln('hands-on').